...
class FaceIdHandler(private val context: Context) {
    ...
    private fun sign(appId: String, userId: String, version: String, ticket: String, nonce: String): String {
        val params = listOf(appId, userId, version, ticket, nonce)
        val sortedParams = params.sorted()
        val concatenatedString = sortedParams.joinToString("")
        val signature = sha1(concatenatedString)
        Log.d(TAG, "签名参数: $params")
        Log.d(TAG, "排序后参数: $sortedParams")
        Log.d(TAG, "拼接字符串: $concatenatedString")
            ...
            WbCloudFaceVerifySdk.getInstance().initSdk(context, bundle,
                object : WbCloudFaceVerifyLoginListener {
                    ...
                    override fun onLoginFailed(error: WbFaceError?) {
                        ...
                        throw Exception("SDK登录失败: ${error?.desc}")
                    }
                })

            Log.d(TAG, "SDK initSdk 调用完成，initSuccess: $initSuccess")
            initSuccess
        } catch (e: Exception) {
            Log.e(TAG, "初始化失败", e)
            false
